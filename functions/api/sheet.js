 ...
 *     APP_PASSWORD  = รหัสสำหรับเข้าเว็บ        ← กด Encrypt ด้วย (ไม่ตั้ง = ไม่ล็อก)
 *
 * ด่านกันคนนอก
 *   ตั้ง APP_PASSWORD แล้ว ทุกคำขอที่เข้ามาตรงนี้ต้องมีคุกกี้ที่ผ่านการตรวจก่อน
 *   ไม่มี = ตอบ 401 กลับไปเฉย ๆ ไม่ยิงต่อไปที่ชีต
 *   URL ของชีตกับ token ยังอยู่ฝั่งเซิร์ฟเวอร์เหมือนเดิม ไม่เคยส่งไปถึงเบราว์เซอร์
 *
 * สิ่งที่ด่านนี้ "ไม่ได้" กัน
 *   ตัวไฟล์หน้าเว็บ (index.html, app.js) ยังโหลดได้โดยไม่ต้องมีรหัส
 *   — แต่ในนั้นไม่มีความลับอะไรเลย มีแต่โครงหน้าจอ ข้อมูลจริงอยู่หลังด่านทั้งหมด
 *   ถ้าอยากล็อกตั้งแต่ตัวหน้าเว็บด้วย ให้เปิด Cloudflare Access (Zero Trust) ทับอีกชั้น
 */

import { passwordOf, isAuthed } from '../_lib/auth.js';

const ALLOWED_ACTIONS = new Set([
  'list', 'metrics', 'leads', 'changes', 'ping',
  'create', 'update', 'delete', 'bulkCreate',
  'saveProducts', 'saveCampaign', 'upsertMetrics', 'upsertLeads', 'upsertChanges'
]);
...
export async function onRequestPost({ request, env }) {
  // ★ ด่านแรก — ยังไม่ผ่านรหัสผ่านก็ไม่ต้องคุยกันต่อ
  //   ตรวจก่อนแตะ SHEET_URL/API_TOKEN ด้วยซ้ำ ความลับจึงไม่ถูกใช้เลยถ้าไม่ผ่าน
  if (!(await isAuthed(request, env))) {
    return json({ ok: false, error: 'ยังไม่ได้เข้าสู่ระบบ', needLogin: true }, 401);
  }
...
export async function onRequestGet({ request, env }) {
  ...
  return json({
    ...
    // หน้าเว็บใช้สองค่านี้ตัดสินว่าจะเด้งหน้ากรอกรหัสไหม
    authRequired: !!passwordOf(env),
